...
class BaseIndexedDB {
  ...
  openDatabase() {
    return new Promise((resolve, reject) => {
      ...
      let UserInfo = ''
      let suffixValue =
        (UserInfo && UserInfo[this.suffixKey]) || localStorage.getItem(this.suffixKey) || ''
      ...
      request.onupgradeneeded = (event) => {
        ...
        objectStoreIndexedDB.forEach(({ primaryKey, storeName, suffixKey }) => {
          let suffixValue =
            (UserInfo && UserInfo[suffixKey]) || localStorage.getItem(suffixKey) || ''
  ...
  async deleteData(key) {
    try {
      ...
      this.deleteDatabase()
      console.log('BaseIndexedDB error:', err)
    }
  }
